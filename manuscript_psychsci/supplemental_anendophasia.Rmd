...
## Internal Verbalizations factor from the Internal Representations Questionnaire (Roebuck & Lupyan, 2020) with factor loadings

```{r, echo=FALSE}
IRQ_items_table <- data.frame(
  Item = c('I think about problems in my mind in the form of a conversation with myself',
  ...
  'Verbal Factor' = c(0.776,0.719,0.699,0.653,0.650, 0.595,0.583,0.559,0.493,0.486,0.486, 0.467))
                        
kable(IRQ_items_table, align='l', booktabs = TRUE,
  longtable = TRUE, linesep = "") %>%
  kable_styling(full_width = T, latex_options = c("hold_position", 'repeat_header'),font_size = 8)
  
```

## Distribution of Verbal Scores on the IRQ with cut-offs (full sample N = 1037)

```{r,echo=FALSE,warning=FALSE}
mturk_ids_1 <- read.csv('../data/mturk_irq_distribution.csv')
sona_ids <- read.csv('../data/sona_irq_distribution.csv')
mturk_ids_2 <- read.csv('../data/mturk_irq_distribution_additional.csv')
irq_total_distribution <- bind_rows(mturk_ids_1 %>% select(workerId, VerbalScored) %>%add_column(source = 'mturk_1'),
                                    mturk_ids_2 %>%select(workerId, VerbalScored) %>% add_column(source = 'mturk_2'),
                                    sona_ids %>% mutate(workerId = as.character(workerId)) %>% select(workerId, VerbalScored)%>% add_column(source = 'sona')) 
minmax <- phon_sim_trials %>% group_by(high_low_verbal) %>% summarise_at(vars(VerbalScored),
               list(min = min, max = max))
ggplot(irq_total_distribution, aes(VerbalScored)) +
  geom_histogram(fill='white',color='black',binwidth = 0.1) +
  theme_bw()+
  geom_vline(xintercept = minmax$min[1], color='#332288', size=1.5) +
  geom_vline(xintercept = minmax$max[2], color='#CC6677', size=1.5) +
  geom_vline(xintercept = quantile(irq_total_distribution$VerbalScored, probs = 0.25), linetype ='dotdash', color='black') +
  geom_vline(xintercept = quantile(irq_total_distribution$VerbalScored, probs = 0.5), linetype ='dotdash', color='black') +
  geom_vline(xintercept = quantile(irq_total_distribution$VerbalScored, probs = 0.75), linetype ='dotdash', color='black') +
  annotate('text', label = '25%-ile', angle=90, x =3.5, y=105)+
  annotate('text', label = '50%-ile', angle=90, x =3.9, y=105)+
  annotate('text', label = '75%-ile', angle=90, x =4.25, y=105)
# attempted recruitment
#quantile(irq_total_distribution$VerbalScored, probs = c(0.30,0.70))

# actual included participants
#quantile(irq_total_distribution$VerbalScored, probs = c(0.20,0.71))
...
## Rhyme judgment items

### Target words characteristics (frequency and name agreement)

```{r name-agreement,echo=FALSE}
rhyme_subtlex <- read.csv('../data/rhyme_judgments_with_freq.csv',row.names = 1)
chance_pairs <- c('bin.png','chin.png', 'cab.png', 'crab.png', 'rake.png', 'cake.png', 'wave.png', 'cave.png', 'park.png', 'shark.png')
rhyme_agreement <- read.csv("../data/rhyming_image_agreement.csv", row.names = 1)
rhyme_agreement <- rhyme_agreement %>% separate(image, c("visual", "target_word"), "/") %>%
   select(-visual)
rhyme_agreement$target_word <- substr(rhyme_agreement$target_word,1,nchar(rhyme_agreement$target_word)-4)
## change square_rhyme to square
rhyme_agreement$target_word[71] <- 'square'
for_merging <- rhyme_subtlex %>%
  select(image_1, image_2, image_1_wordFreq, image_2_wordFreq) %>%
  distinct() %>%
  pivot_longer(c(image_1, image_2), names_to = 'target_word_frequency', values_to = 'target_word') %>%
  mutate(frequency = if_else(target_word_frequency == 'image_1', image_1_wordFreq, 
                             if_else(target_word_frequency == 'image_2', image_2_wordFreq, NA))) %>%
  select(target_word, frequency) %>%
  distinct()
rhyme_agreement <- merge(rhyme_agreement, for_merging, by = 'target_word')
kable(rhyme_agreement, align='l', escape = T, booktabs = TRUE,
      caption = 'Materials for the rhyme judgment experiment including name agreement (0 to 1) from a separate validation experiment and target word frequency (SUBTLEX log-transformed counts).',
  linesep = "", col.names = c('Target word', 'Name agreement', 'Target word frequency'),longtable = TRUE) %>%
...
rhyme_subtlex <- rhyme_subtlex %>% filter(rt > 200)
rhyme_subtlex$type <- as.factor(rhyme_subtlex$type)
contrasts(rhyme_subtlex$type) = contr.sum(3) 
rhyme_subtlex <- rhyme_subtlex %>% mutate(stim_pair = paste(pmax(as.character(image_1),as.character(image_2)),pmin(as.character(image_1), as.character(image_2))))

# test if there were differences in frequency across typ
rhyme_subtlex %>%
  pivot_longer(c(image_1_wordFreq, image_2_wordFreq), values_to = 'subtlex_freq', names_to = 'word1_and_2') %>%
  lmer(subtlex_freq~type + (1|stim_pair),data=.) %>%
  summary
# no differences

# check whether frequency difference between the two target words on a given trial differ across conditions
rhyme_subtlex %>%
  select(freq_diff, type) %>%
  distinct() %>%
  lm(freq_diff ~ type,data=.) %>%
  summary
# no differences

```
Using linear models, we tested whether there were any differences in mean frequency (SUBTLEX log-transformed counts) between the three rhyming conditions (including random intercepts for stimulus pair) and whether the three conditions differed in the absolute difference between the modal name of the first and second picture. Target word frequency did not differ between conditions (both p > .391) and neither did absolute difference in frequency between the two target words in a given stimulus pair (both p > .155).
