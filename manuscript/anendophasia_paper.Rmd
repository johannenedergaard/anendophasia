...
abstract: |
  It is commonly assumed that inner speech – the experience of thought as natural language – is both universal and ubiquitous. Recent evidence, however, suggests that similar to some other phenomenal experiences like visual imagery, the experience of inner speech ranges frfom constant to non-existent. We propose a name for the experience of lacking inner speech – anendophasia – and report four behavioral studies examining some of its behavioral consequences. People reporting low levels of inner speech were found to have lower performance on a verbal working memory task and rhyme judgments from images and were slightly slower at visual same/different judgments. Task switching performance, previously linked to endogenous verbal cueing, was unaffected by differences in inner speech experience. We also report results of a questionnaire showing anendophasia to be associated with a range of experiential differences ranging from experiencing earworms to memory for conversations. Studies of anendophasia, together with aphantasia, synesthesia, and differences in autobiographical memory are providing glimpses into what may be a much larger space of hitherto unexplored differences in people’s phenomenal experience. 
...
Judging by these accounts, there are important differences in the extent to which people experience an inner voice. Whether these differences in experience result in differences in behavior is still an open question which we begin to address in the present article.
...
# Methods
## Participants
We contacted participants who had previously completed the Internal Representations Questionnaire [@roebuck2020internal] and recruited ones with the lowest and highest scores on verbal representation. This study received ethical approval from XXXX. We will use the shorthand ‘high-verbal’ and ‘low-verbal’ to refer to these two groups although it should be noted that these just refer to their inclination or propensity to engage in verbal mental imagery and not to their general linguistic abilities. All four experiments were conducted using custom-written software with the JavaScript package jsPsych version 6 [@de2015jspsych].
...
VWM_score_p <- ggplot(phonsim_desc_df_score, aes(original_word_set, score, color=high_low_verbal)) +
  ...
  scale_color_manual('Group: Color',values = color_palette[c(4,6)], labels=c('More inner speech', 'Less inner speech'))
  #scale_shape_manual('Group: Shape', values= c(17,19),labels=c('High-verbal', 'Low-verbal'))+
...
ggplot(phonsim_desc_df_TOL, aes(talk_out_loud, score, color=high_low_verbal,shape=high_low_verbal)) +
  ...
  scale_color_manual('Group: Color',values = color_palette[c(4,6)], labels=c('More inner speech', 'Less inner speech'))+
  scale_shape_manual('Group: Shape',values = c(17,19), labels=c('More inner speech', 'Less inner speech'))+
  guides(shape = guide_legend(override.aes = list(size=3)))+
...
```{r strategies-vwm, echo=FALSE}
strategies_vwm <- data.frame(`More inner speech` = c("Remembering the order of the first letters once the words were familiar (e.g. c, b, t, r, d for 'cough', 'bough', 'through', 'rough', 'dough'). One participant reported this.","Finding a cadence/melody and using this to repeat the words.", "Chunking.", "Hand and body gestures.", "Creating a story or a sentence with the words in order (both visual and verbal). This one was the most common strategy."),
                                `Less inner speech` = c("Remembering the order of the first letters once the words were familiar (e.g. c, b, t, r, d for 'cough', 'bough', 'through', 'rough', 'dough'). This strategy was much more common for the low-verbal group than for the high-verbal group.", "Form a story or a narrative. This was a less common strategy than remembering the first letters.", '','',''))
strategies_vwm %>%
  kable(caption = 'Alternative strategies used in the verbal working memory experiment', booktabs=T, col.names = c('More inner speech', 'Less inner speech')) %>%
  row_spec(row = 0, bold = T) %>%
...
As for log-transformed reaction time, there was a significant effect of condition difficulty with more difficult conditions being associated with slower reaction times ($\beta$ = `r round(summary(switching_rt_m)$coefficients[2,1],2)`; SE = `r round(summary(switching_rt_m)$coefficients[2,2],2)`; z = `r round(summary(switching_rt_m)$coefficients[2,4],2)`; p < .001) but no effect of verbal score and no interaction effects (both p > `r round(summary(switching_rt_m)$coefficients[3,5],3)`).
...
ggplot(aes(condition, switching_is_correct, color=high_low_verbal,shape=high_low_verbal)) +
  #geom_sina(data= task_switch_correct_df_individual, aes(condition, switching_is_correct), alpha=0.3)+
  geom_line(data= task_switch_correct_df_individual, aes(condition, switching_is_correct, group=worker_id), alpha=0.2)+
  geom_errorbar(aes(ymin=switching_is_correct-ci, ymax=switching_is_correct+ci), width=.1, position= pd) +  
  stat_summary(fun = mean, geom = 'point', aes(group = high_low_verbal), position= pd) +
  stat_summary(fun = mean, geom = 'line', aes(group = high_low_verbal), size = 1, position= pd)+
  theme_bw() +
...
ggplot(aes(condition, rt, color=high_low_verbal, shape=high_low_verbal)) +
  #geom_sina(data= task_switch_rt_df_individual, aes(condition, rt), alpha=0.3)+
...
ggplot(subset(SD_rt_df_catdog, cat_or_dog != 'cat-dog'), aes(cat_or_dog, rt, color=high_low_verbal)) +
  ...
  geom_line(data= subset(SD_rt_df_catdog_individual, cat_or_dog != 'cat-dog'), aes(cat_or_dog, rt,group=worker_id), alpha=0.2)+
  geom_errorbar(aes(ymin=rt-ci, ymax=rt+ci), width=.1, position= pd) +  
  stat_summary(fun = mean, geom = 'point', aes(group = high_low_verbal),position= pd) +
  stat_summary(fun = mean, geom = 'line', aes(group = high_low_verbal), size = 1, position= pd)+
  theme_bw() +
...
```{r, include=FALSE}
Q_anendophasia_numeric <- read.csv('survey_data/qualtrics_numeric_anendophasia.csv', row.names = 1)
Q_anendophasia_text <- read.csv('survey_data/qualtrics_text_anendophasia.csv', row.names = 1)
