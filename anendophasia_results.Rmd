...
SD_rt_df <- same_different_trials %>%
  filter(correct == 1) %>%
  summarySEwithin(measurevar = 'rt', betweenvars = c('high_low_verbal'),
                  withinvars = 'judgment_type',idvar = 'worker_id')
SD_rt_df_individual <- same_different_trials %>%
  filter(correct == 1) %>%
  summarySEwithin(measurevar = 'rt', betweenvars = c('high_low_verbal', 'worker_id'),
                  withinvars = 'judgment_type',idvar = 'worker_id')
```
...
rhyme_desc_df_rt <- rhyming_trials %>%
  summarySEwithin(measurevar = c('rt'), betweenvars = 'high_low_verbal',
                  withinvars = 'type', idvar='worker_id', na.rm=T)
rhyme_desc_df_correct <- rhyming_trials %>%
  summarySEwithin(measurevar = 'correct', betweenvars = 'high_low_verbal',
...
colnames(rhyme_desc_df)[7] <- 'ci_rt'
colnames(rhyme_desc_df)[14] <- 'ci_accuracy'
...
rhyme_correct_df <- rhyming_trials %>%
  summarySEwithin(measurevar = 'correct', betweenvars = c('high_low_verbal'),
                  withinvars = c('type'),idvar = 'worker_id')
rhyme_correct_df_individual <- rhyming_trials %>%
  summarySEwithin(measurevar = 'correct', betweenvars = c('high_low_verbal', 'worker_id'),  withinvars = c('type'),idvar = 'worker_id')
```
...
```{r, echo=FALSE}
ggplot(rhyme_rt_df, aes(type, rt, color=high_low_verbal)) +
...
```
```{r, echo=FALSE}
ggplot(rhyme_correct_df, aes(type, correct, color=high_low_verbal)) +
...
rhyme_acc_agreement <- glmer(correct~ name_agreement_img1 *VerbalScored + (1|worker_id), family='binomial',
                      subset(rhyming_trials))
summary(rhyme_acc_agreement)
...
ggplot(rhyme_correct_df, aes(talk_out_loud, mean_correct, color=high_low_verbal)) +
  geom_sina(alpha=0.3)+
  #stat_summary(geom='line',fun='mean',alpha=0.3, aes(group = worker_id, color = high_low_verbal)) +
  stat_summary(fun = mean, geom = 'point', aes(group = high_low_verbal)) +
  stat_summary(fun = mean, geom = 'line', aes(group = high_low_verbal), size = 1)+
...
```{r}
task_switch_desc_df %>%
  dplyr::mutate(mean_correct = mean_correct*100) %>%
  kable(digits=2) %>%
  kable_styling(bootstrap_options = "striped")
```
```{r, echo=FALSE}
...
```{r, echo=FALSE}
task_switch_rt_df %>% dplyr::mutate(condition = fct_relevel(condition, 
            "addition", "subtraction", "symbolcue", 
            "colorcue", "uncued")) %>%
ggplot(aes(condition, mean_rt, color=high_low_verbal)) +
  geom_sina(alpha=0.3)+
...
```
```{r, echo=FALSE}
task_switch_correct_df %>% dplyr::mutate(condition = fct_relevel(condition, 
            "addition", "subtraction", "symbolcue", 
            "colorcue", "uncued")) %>%
ggplot(aes(condition, mean_correct, color=high_low_verbal)) +
  geom_sina(alpha=0.3)+
...
corrplot(correlations_hv$r, method="color",addgrid.col='black',p.mat=ps_hv, sig.level = 0.05,insig='blank', tl.cex = 0.5)
...
corrplot(correlations_lv$r, method="color", addgrid.col='black',p.mat=ps_lv, sig.level = 0.05,insig='blank', tl.cex = 0.5)
